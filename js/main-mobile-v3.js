(() => {
  ...
  // Em navegadores internos (Linktree/Instagram/Facebook), a página mobile
  // rola dentro de .page-shell. Isso impede que o viewport seja arrastado
  // horizontalmente e revele uma área vazia/preta.
  document.querySelectorAll('a[href^="#"]').forEach(link => {
    const href = link.getAttribute('href');
    if (!href || href === '#') return;
    link.addEventListener('click', event => {
      const target = document.querySelector(href);
      if (!target || !pageShell || innerWidth > 768) return;
  ...
  pageShell?.addEventListener('scroll', reveal, { passive: true });
})();
